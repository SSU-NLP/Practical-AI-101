...
INSERTS = [
    ...
    ("단어를 임베딩으로 변환하기", "임베딩 룩업 (nn.Embedding)", "EmbeddingLookup",
     "nn.Embedding은 학습되는 룩업 테이블입니다 — 단어 → 인덱스 → 테이블의 해당 행 → 그 행이 곧 임베딩 벡터."),
    ("모델 설계 — 임베딩 평균 풀링", "평균 풀링 (Mean Pooling)", "MeanPooling",
    ...
    ("임베딩이 학습한 *문맥*", "임베딩 학습 과정 — 문맥이 가르친다", "EmbeddingTraining",
